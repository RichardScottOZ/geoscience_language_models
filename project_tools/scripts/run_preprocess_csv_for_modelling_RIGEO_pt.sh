...
PRE_PIPELINE='PIPELINE_GLOVE_PLUS_PT'
...
POST_PIPELINE='POSTPIPE_GLOVE_PT'
...
then 
    echo "Dataset RIGEO..."
    INPUT_DIRS=(
        '/i/Brazil/RIGEO_pdfout'
    )
    OUTPUT_DIR='/i/Brazil/RIGEO_processed'
    PARTIAL_OUTPUT_DIR='/i/Brazil/RIGEO_processed'
fi
...
then 
    ...
    do
        ...
        do
            ...
            CMD="python scripts/preprocess_csv_for_modelling_pt.py --INPUT_DIRS ${partial_folder} --N_FILES ${N_FILES} --PARTIAL_OUTPUT_DIR ${PARTIAL_OUTPUT_DIR} --OUTPUT_DIR ${OUTPUT_DIR} --PREPROCESSING_PIPELINE ${PRE_PIPELINE} --POST_PIPELINE ${POST_PIPELINE} --SUFFIX ${SUFFIX} --NO_FINAL_FILE ${IS_PARTIAL} --PERC_FILE_START ${perc_start} --PERC_FILE_END ${perc_end}"
            echo ${CMD}
    ...
    perc_start=-1
    perc_end=-1
    CMD="python scripts/preprocess_csv_for_modelling_pt.py --INPUT_DIRS ${INPUT_DIRS[@]} --N_FILES ${N_FILES} --PARTIAL_OUTPUT_DIR ${PARTIAL_OUTPUT_DIR} --OUTPUT_DIR ${OUTPUT_DIR} --PREPROCESSING_PIPELINE ${PRE_PIPELINE} --POST_PIPELINE ${POST_PIPELINE} --SUFFIX ${SUFFIX} --NO_FINAL_FILE ${IS_PARTIAL}"
    echo ${CMD}
